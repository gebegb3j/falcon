...
void ArgManager::usage(Args& args, const std::string& prog) {
  printf("Usage: %s [aAcCdfgilnoOpPrsStTvyY] -f rx_frequency (in Hz) | -i input_file\n", prog.c_str());
#ifndef DISABLE_RF
  ...
  printf("\t-d disable ASCII plots [Default enabled]\n");
...
void ArgManager::parseArgs(Args& args, int argc, char **argv) {
  int opt;
  defaultArgs(args);
  while ((opt = getopt(argc, argv, "aAcCdDEfgilnpPrsStTvyY")) != -1) {
    switch (opt) {
      case 'a':
