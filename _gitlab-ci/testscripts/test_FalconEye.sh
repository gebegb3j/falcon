...
uut="src/FalconEye"
uut_params="-i $templatedir/template-iq.bin -p 50 -P 2 -c 392 -n 1000 -D $workdir/test-falcon-dci.csv -E $workdir/test-falcon-stats.csv -d"
...
cat $workdir/test-falcon-dci.csv | cut -f2- | rev | cut -f4- | rev > $workdir/stripped-test-falcon-dci.csv
cat $templatedir/template-falcon-dci.csv | cut -f2- | rev | cut -f4- | rev > $workdir/stripped-template-falcon-dci.csv
