...
void parse_args(prog_args_t *args, int argc, char **argv) {
  int opt;
  args_default(args);
  while ((opt = getopt(argc, argv, "aADEoglipPcOCtdnvfuUsSyYzZ")) != -1) {
    switch (opt) {
      case 'i':
          ...
          if (ue_dl.current_rnti != 0xffff) {
            ...
            switch(n) {
              case 0:
                ...
                break;
              case 40:
                t_rnti = 256*(uint16_t)pch_payload[(n/8)-2] + pch_payload[(n/8)-1];
                srslte_ue_dl_reset_rnti_user(&falcon_ue_dl, t_rnti);
                rnti_manager_activate_and_refresh(falcon_ue_dl.rnti_manager, t_rnti, 0, RM_ACT_RAR);
              ...
              case 56:
                ...
                break;
              case 72:
                t_rnti = 256*(uint16_t)pch_payload[(n/8)-4] + pch_payload[(n/8)-3];
                srslte_ue_dl_reset_rnti_user(&falcon_ue_dl, t_rnti);
                rnti_manager_activate_and_refresh(falcon_ue_dl.rnti_manager, t_rnti, 0, RM_ACT_RAR);
                ...
                break;
              case 120:
                t_rnti = 256*(uint16_t)pch_payload[(n/8)-3] + pch_payload[(n/8)-2];
                srslte_ue_dl_reset_rnti_user(&falcon_ue_dl, t_rnti);
                rnti_manager_activate_and_refresh(falcon_ue_dl.rnti_manager, t_rnti, 0, RM_ACT_RAR);

                t_rnti = 256*(uint16_t)pch_payload[(n/8)-9] + pch_payload[(n/8)-8];
                srslte_ue_dl_reset_rnti_user(&falcon_ue_dl, t_rnti);
                rnti_manager_activate_and_refresh(falcon_ue_dl.rnti_manager, t_rnti, 0, RM_ACT_RAR);
